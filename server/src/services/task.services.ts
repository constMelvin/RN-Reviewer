...
export const TaskService = {
  ...
  updateTaskHandler: async ({ dbClient, task_id, values }: UpdateTaskArgs) => {
    return await TaskData.updateTask({ dbClient, task_id, values });
  },
  deleteHandler: async ({ dbClient, task_id }: DeleteTaskArgs) => {
    return await TaskData.deleteTask({ dbClient, task_id });
  },
};
